        ...
        for training_file in training_files.values:
            ...
            if self.audio_path+file_name_no_extension+self.audio_extension  in glob.glob(self.audio_path+"*"+self.audio_extension):
                ...
                for index, row in df.iterrows():
                    ...
                    count_0 = sum(1 for y in Y_calls if y == 0)
                    count_1 = sum(1 for y in Y_calls if y == 1)
                    
                    # Break the loop if we have enough samples of both labels
                    if count_0 >= 1500 and count_1 >= 2000:
                        break
